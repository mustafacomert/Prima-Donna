            ...
            if (allreadyCalculated == true)
            {
                ...
                if (addTorques)
                {
                    for (int i = 0; i < splinters.Count; i++)
                     {
                            Rigidbody rbx = splinters[i].GetComponent<Rigidbody>();
                            if (rbx)
                                rbx.AddTorque(new Vector3(Random.value > 0.5f ? Random.value * 50 : -Random.value * 50, Random.value > 0.5f ? Random.value * 50 : -Random.value * 50, Random.value > 0.5f ? Random.value * 50 : -Random.value * 50));
                    }
                }
            }
            else
